...
public class AthenaScoreboard {
	private static final ScoreboardManager manager = Athena.getInstance().getServer().getScoreboardManager();
	...
	public AthenaScoreboard(String title) {
		this(title, title, Collections.emptyList());
	}

	public AthenaScoreboard(String title, Player player) {
		this(title, title, Collections.singletonList(player));
	}

	public AthenaScoreboard(String title, Collection<? extends Player> players) {
		this(title, title, players);
	}

	public AthenaScoreboard(String id, String title) {
		this(id, title, Collections.emptyList());
	}

	public AthenaScoreboard(String id, String title, Player player) {
		this(id, title, Collections.singletonList(player));
	}

	public AthenaScoreboard(String id, String title, Collection<? extends Player> players) {
		try { scoreboard.removeObjective(StringUtils.left(id, 16)); } catch (Exception ignore) {}
		objective = scoreboard.createObjective(StringUtils.left(id, 16), colorize(title), DisplaySlot.SIDEBAR, false);
