...
import net.aegis.athena.utils.CitizensUtils.NPCFinder;
...
public final class WorldGuardUtils {
	...
	public boolean isInRegion(@NotNull Player player, @NotNull String region) {
		return isInRegion(player.getPlayer().getLocation(), region);
	}

	public boolean isInRegion(@NotNull Player player, @NotNull ProtectedRegion region) {
		return isInRegion(player.getPlayer().getLocation(), region);
	}
